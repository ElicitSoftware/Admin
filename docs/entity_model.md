...
A role granted to a user. Serves as the database fallback when the identity provider supplies no application role (UC-001), and, when `elicit.authorization.mode=DATABASE`, is directly managed by an administrator through the Edit User admin UI (UC-016). A user holds at most one ladder role (`elicit_admin`, `elicit_user`, `elicit_importer`) and, independently, may hold `elicit_analytics` (UC-020).

| Attribute | Description                     | Data Type | Length/Precision | Validation Rules                    |
|-----------|---------------------------------|-----------|------------------|-------------------------------------|
| userId    | User the role is granted to     | Long      |                  | Not Null, Foreign Key (users.id)    |
| roleName  | Granted role name               | String    | 100              | Not Null, Check: one of `elicit_admin`, `elicit_user`, `elicit_importer`, `elicit_analytics` |
