...
5. The system determines the user's effective roles: if the OIDC identity already carries an Elicit role, those roles are used; otherwise the system looks up roles for the username in the local role table.
...
**Trigger:** The OIDC identity carries none of the Elicit roles (step 5).
**Flow:**

1. The system reads the user's roles from the local role table for the active username.
2. The effective identity is stamped as database-sourced and granted those roles.
...
Derived from `application.properties` (OIDC and HTTP permission configuration), `RoleSecurityIdentityAugmentor`, `UserRoleDatabaseLookup`, `LoginView`, `LogoutView`, `UnauthorizedView`, and `AccessDeniedErrorView`.
